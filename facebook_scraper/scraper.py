"""Facebook scraper — implements the BaseCommentScraper hooks."""
...
from .profile import collect_recent_posts, extract_page_name, posts_from_urls

logger = logging.getLogger(__name__)


class FacebookCommentScraper(BaseCommentScraper):
    output_prefix = "facebook"
    ...
    def challenge_message(self) -> str:
        return "Facebook checkpoint active; resolve manually or refresh cookies"
    ...
    def scrape_post(self, driver, post: PostRef) -> dict:
        return scrape_post_comments(
            ...
            self.config,
            post.url,
            feedback_id=post.media_id,
            post_meta=post.metadata,
        )

    def profile_label(self) -> str:
        return extract_page_name(self.config.profile_url)
