...
from .interceptors import inject_interceptor
...
def _apply_chrome_options(options, config: ScraperConfig, fp: BrowserFingerprint) -> None:
    in_docker = is_docker()
...
def build_driver(config: ScraperConfig, fingerprint: BrowserFingerprint | None = None) -> uc.Chrome:
